...
openai.api_key = "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"
    ...
    try:
        ...
        if refined_result:
            user_review_collection.update_one(
                {"_id": review_id},
                {
                    "$set": {"review.output": refined_result},
                    "$push": {"review.history": {"correction_type": correction_type, "output": refined_result}}
                }
            )
